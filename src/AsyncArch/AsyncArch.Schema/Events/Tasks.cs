...
public static class Tasks
{
    ...
    public class Created_V2
    {
        ...
        {
            this.event_version = event_version != 2 ? throw new SchemaException(nameof(event_version)) : event_version;
            this.event_name = event_name != Kind ? throw new SchemaException(nameof(event_name)) : event_name;
        ...
        public class Data
        {
            public Guid task_uuid { get; }
            public Guid assignee { get; }
            public string jira_id { get; }
            public string title { get; }
            public string? description { get; }
            public bool done { get; }

            [JsonConstructor]
            public Data(Guid task_uuid, Guid assignee, string title, string? description, bool done, string jira_id)
            {
                if (jira_id.Contains('[') || jira_id.Contains(']'))
                    throw new ArgumentException("jira_id cannot contain square brackets", nameof(jira_id));
